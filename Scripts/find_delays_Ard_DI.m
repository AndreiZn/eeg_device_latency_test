...
image_appearance_time_s = str2num(setup_labels{2,1});
event_length_s = str2num(setup_labels{3,1});
setup_labels = setup_labels(4:size(setup_labels,1)-1,1);
fclose(fid);
...
for filei=1:numel(files)
    ...
    % get setup idx
    setup_idx = get_setup_idx(filei, trials_per_setting);
    
    % get the total event length (image appearance + inner interval) and the length of image appearance
    CFG.event_length_s = event_length_s(setup_idx);
    CFG.image_appearance_time_s = image_appearance_time_s(setup_idx);
    ...
    if visualize_delay_over_time_flag
        figure();
...
title('Digital input delay relative to arduino')
...
title('GroupID input delay relative to arduino')
